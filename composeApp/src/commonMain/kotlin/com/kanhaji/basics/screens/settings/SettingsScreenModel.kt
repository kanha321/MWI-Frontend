...
object SettingsScreenModel : ScreenModel {
    ...
    private fun updateDynamicColorSetting(enabled: Boolean) {
        ...
        screenModelScope.launch {
            // Update the color immediately for smooth UI
            if (enabled) {
                ThemeManager.customColor.value = ThemeManager.defaultSeed
            } else {
                val savedCustomColor = PrefsManager.getString(PrefsResources.CUSTOM_COLOR)
                if (savedCustomColor != null) {
                    ThemeManager.customColor.value = hexToColor(savedCustomColor)
                }
            }
